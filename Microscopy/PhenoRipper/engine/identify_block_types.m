...
if(isTissue)
    channels_per_file=filenames(1).tImg.numberOfChannels;
    number_of_training_images=length(filenames);
    xres_full=max(arrayfun(@(x) diff(x.pixelRegion{1})+1 ,filenames));
    yres_full=max(arrayfun(@(x) diff(x.pixelRegion{2})+1 ,filenames));
else
